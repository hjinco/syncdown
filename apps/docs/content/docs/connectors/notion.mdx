...
- discovers shared Notion pages
- discovers shared data sources
- queries pages inside each shared data source
- fetches page metadata and Markdown for each candidate page

Pages from shared data sources are written into database-specific directories when the data source name is available.

Rendered Notion Markdown includes YAML frontmatter with:

- common fields such as `title`, `source`, `created`, `updated`
- `database` when the page came from a shared data source
...
For shared data sources, that cursor is used to request incremental page candidates. Shared top-level pages are still discovered on each run.
...
- Only content shared with the selected integration or OAuth-connected account is visible.
- The current implementation targets shared pages and shared data sources, not every possible Notion object type.
